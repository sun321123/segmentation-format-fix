...
#-----------------------------------------------------------------------------------#
Origin_SegmentationClass_path   = "SegmentationClass"
Out_SegmentationClass_path      = "SegmentationClass_new"
mmcv.mkdir_or_exist(Out_SegmentationClass_path)
#-----------------------------------------------------------------------------------#
...
if __name__ == "__main__":
    ...
    for png_name in tqdm(png_names):
        ...
        w, h    = png.size
        png = np.array(png)
        #在制作自己的数据集时，有些图像中的非255像素值比较多且不固定，所以不能单纯使用[0，255]进行变换，下面的代码用于将图片中所有的不为0的像素值统一修改为1
        res = set(png.flatten().tolist())
        res = list(res)
        Origin_Point_Value = res
        Out_Point_Value = Out_Point_Value + [1] * (len(res) - 1)
        Origin_Point_Value = np.array(Origin_Point_Value)
        Out_Point_Value = np.array(Out_Point_Value)
